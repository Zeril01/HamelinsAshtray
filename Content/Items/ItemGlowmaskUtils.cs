    ...
    public static class ItemGlowmaskUtils
    {
        public static void DrawItemGlowmask(Texture2D texture, Terraria.DataStructures.PlayerDrawSet info)
        {
            ...
            if (info.shadow != 0f || info.drawPlayer.frozen || (info.drawPlayer.itemAnimation <= 0 || item.useStyle == ItemUseStyleID.None)
                && (item.holdStyle <= 0 || info.drawPlayer.pulley) || info.drawPlayer.dead || item.noUseGraphic || info.drawPlayer.wet && item.noWet) return;
            ...
            info.DrawDataCache.Add(new(texture,
                new((int)(info.ItemLocation.X - Main.screenPosition.X + offset.X), (int)(info.ItemLocation.Y - Main.screenPosition.Y + offset.Y)),
